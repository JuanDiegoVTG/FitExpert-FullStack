...
public class ClienteController {
    ...
    public String mostrarFormularioEdicionPerfil(Authentication auth, Model model) {
        ...
        // Rellenamos datos básicos del Usuario (estos siempre existen)
        dto.setNombreCompleto(usuarioLogueado.getNombres() + " " + usuarioLogueado.getApellidos());
        dto.setEmail(usuarioLogueado.getEmail());
        dto.setTelefono(usuarioLogueado.getTelefono());

        // Solo si el perfil existe, pasamos sus datos al DTO
        if (perfilEntidad != null) {
...
public String actualizarPerfil(@Valid @ModelAttribute("perfil") ClientePerfilDTO perfilForm,
    ...
    try {
        ...
        // Si el usuario es nuevo y no tiene perfil, lo creamos
        if (perfilEntidad == null) {
            perfilEntidad = new Perfil();
            perfilEntidad.setUsuario(usuarioActual); // Muy importante para la relación @OneToOne
            perfilEntidad.setNombreCompleto(usuarioActual.getNombres() + " " + usuarioActual.getApellidos());
        }

        // Sincronizamos datos del Usuario
        usuarioActual.setEmail(perfilForm.getEmail());
        usuarioActual.setTelefono(perfilForm.getTelefono() != null ? perfilForm.getTelefono().replace(" ", "") : null);
        
        // Sincronizamos datos del Perfil
        ...
        // Guardamos el usuario (esto guardará el perfil en cascada )
        // O guarda ambos 
        usuarioActual.setPerfil(perfilEntidad);
